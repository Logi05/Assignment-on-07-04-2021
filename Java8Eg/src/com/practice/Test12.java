	...
	public static void main(String[] args) {
		...
		for (Product p : list) {
			if (bp1.test(p.laseprice, p.shocksprice)) {
				System.out.println("Size: "+p.size + " " +"laseprice: "+ p.laseprice + " " +"shocksprice: "+ p.shocksprice);
		...
		for (Product p1 : list) {
			if (((p1.size) >= 5) && ((p1.laseprice + p1.shocksprice) <= 50)) {
			    System.out.println("Size: "+p1.size + " " +"laseprice: " + p1.laseprice + " " +"shocksprice: "+ p1.shocksprice);
